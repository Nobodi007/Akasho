...
def render_tab3(cfg: dict[str, Any], data: pd.DataFrame, data_err: Optional[str],
    ...
    with left:
        ...
        port_val_thb = sum(q * wallet_price.get(sym, 0.0) for sym, q in coins_book.items())
...
def main() -> None:
    ...
    with tab4:
        ...
        st.multiselect(
            "⭐ เลือกเหรียญที่ต้องการปักเโอเคครับ นี่คือไฟล์เต็มที่รวม 4 patch เข้าไปแล้วทั้งหมด ก็อปแทนที่ไฟล์เดิมได้เลย
